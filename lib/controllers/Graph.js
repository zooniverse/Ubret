(function() {
  ...
  Graph = (function(_super) {
    ...
    Graph.prototype.createYAxis = function(ticks, label, format) {
      ...
      if (label !== '') {
        return this.svg.append('text').attr('class', 'y label').attr('text-anchor', 'middle').attr('x', 0).attr('y', this.graphHeight / 2).attr('transform', 'rotate(-90)').text(this.prettyKey(label));
      }
    };

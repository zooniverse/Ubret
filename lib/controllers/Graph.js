(function() {
  ...
  Graph = (function(_super) {
    ...
    Graph.prototype.createYAxis = function(ticks, label, format) {
      ...
      if (label !== '') {
        return this.svg.append('text').attr('class', 'y label').attr('text-anchor', 'middle').attr('y', -30).attr('x', -(this.graphHeight / 2)).attr('transform', 'rotate(-90)').text(this.prettyKey(label));
      }
    };
    ...
    Graph.prototype.createGraph = function() {
      ...
      this.svg = d3.select("#" + this.channel + " svg").attr('width', this.width).attr('height', this.height).append('g').attr('transform', "translate(" + margin.left + ", " + margin.right + ")");

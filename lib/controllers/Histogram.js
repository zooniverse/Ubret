(function() {
  ...
  Histogram = (function(_super) {
    ...
    Histogram.prototype.createGraph = function() {
      var bar, data, formatCount, height, margin, svg, values, width, x, xAxis, y, yAxis,
        _this = this;
      if (!this.data.length) {
      ...
      x = d3.scale.linear().domain([Math.floor(d3.min(values)), Math.ceil(d3.max(values))]).range([width, 0]);

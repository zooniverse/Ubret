  ...
  Histogram = (function(_super) {
    ...
    Histogram.prototype.createGraph = function() {
      ...
      bar = svg.selectAll(".bar").data(data).enter().append('g').attr('class', 'bar').attr('transform', function(d) {
        return "translate(" + (x(d.x) - 1) + ", " + (y(d.y)) + ")";
      });
      ...
      svg.append('text').attr('class', 'x label').attr('text-anchor', 'middle').attr('x', width / 2).attr('y', height + 35).text(this.prettyKey(this.variable));
      return svg.append('text').attr('class', 'title').attr('text-anchor', 'middle').attr('x', width / 2).attr('y', 0).text("Distribution of " + (this.prettyKey(this.variable)));
    };

(function() {
  ...
  Histogram = (function(_super) {
    ...
    Histogram.prototype.createGraph = function() {
      var bar, data, datum, formatCount, height, svg, values, width, x, xAxis, xValues, y, yAxis, _i, _len,
        _this = this;
      if (!this.data.length) {
      ...
      console.log(xValues);
      xAxis = d3.svg.axis().scale(x).orient('bottom').tickValues(values).tickFormat(d3.format(",.02f"));

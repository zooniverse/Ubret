(function() {
  ...
  Histogram = (function(_super) {
    ...
    Histogram.prototype.createGraph = function() {
      ...
      svg = d3.select("#" + this.channel + " svg").attr('width', this.width).attr('height', this.height).append('g').attr('transform', "translate(" + this.margin.left + ", " + this.margin.bottom + ")");
      ...
      if (bins.length !== 0) {
        ...
        return bar.append('text').attr("dy", ".75em").attr("y", 6).attr("x", (this.x(bins[1].x) - x(bins[0].x)) / 2).attr("text-anchor", "middle").text(function(d) {

(function() {
  ...
  Scatterplot = (function(_super) {
    ...
    Scatterplot.prototype.createGraph = function() {
      var data, format, graphHeight, graphWidth, point, svg, x, xAxis, xDomain, y, yAxis, yDomain,
        _this = this;
      ...
      if (typeof this.xAxisKey !== 'undefined') {
        format = data.length === 0 ? ',.02f' : this.xFormat;
        x = d3.scale.linear().domain(xDomain).range([0, graphWidth]);
        xAxis = d3.svg.axis().scale(x).orient('bottom').tickFormat(format);
        if (data.length !== 0) {
          xAxis.ticks(this.calculateTicks(x));
        }

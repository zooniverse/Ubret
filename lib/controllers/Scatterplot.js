(function() {
  ...
  Scatterplot = (function(_super) {
    ...
    Scatterplot.prototype.createGraph = function() {
      ...
      this.drawAxes();
      console.log('here');
      this.drawPoints(this.data, this.color);
      return console.log('at the end');
    };

    Scatterplot.prototype.drawAxes = function() {
      var xAxis, xDomain, yAxis, yDomain,
        _this = this;
      if (this.filteredData.length !== 0) {
        this.data = _.map(this.filteredData, function(d) {
          return {
            x: d[_this.xAxisKey],
            y: d[_this.yAxisKey]
          };
      ...
      if (typeof this.xAxisKey !== 'undefined') {
        ...
        this.svg.append('g').attr('class', 'x axis').attr('transform', "translate(0, " + graphHeight + ")").call(xAxis);
      ...
      if (typeof this.yAxisKey !== 'undefined') {
        ...
        return this.svg.append('text').attr('class', 'y label').attr('text-anchor', 'middle').attr('y', -40).attr('x', -(this.graphHeight / 2)).attr('transform', "rotate(-90)").text(this.prettyKey(this.yAxisKey));
      }
    };

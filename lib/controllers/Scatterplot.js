  ...
  Scatterplot = (function(_super) {
    ...
    Scatterplot.prototype.createGraph = function() {
      ...
      if (this.filteredData.length !== 0) {
        ...
        yDomain = d3.extend(data, function(d) {
          return d.y;
        });
      } else {
        data = [];
      ...
      if (typeof this.yAxisKey !== 'undefined') {
        ...
        svg.append('text').attr('class', 'y label').attr('text-anchor', 'middle').attr('x', -30).attr('y', graphHeight / 2).attr('transform', "rotate(-90)").text(this.prettyKey(this.yAxisKey));
      }
      if (data.length !== 0) {
        point = svg.selectAll('.point').data(data).enter().append('g').attr('class', 'point').attr('transform', "translate(" + (x(d.x)) + ", " + (y(d.y)) + ")").on('mouseover', this.displayTooltip);
        return point.append('circle').attr('r', 3);
      }
    };
